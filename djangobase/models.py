...
class Node(models.Model):
    ...
    @property
    def uri(self):
        return "%s:%s:%s" % (settings.DJANGOBASE_URI_PROTOCOL, self.type, self.id)
...
class Entity(Node):
    ...
    def to_dict(self, *args, **kwargs):
        obj = super(Entity, self).to_dict(*args, **kwargs)
        return dict(
            name=self.name,
    ...
    def generate_colors(self, save=True):
        image = self.image
        color_thief = None
        if self.image:
            color_thief = ColorThief(self.image.path)
        elif self.image_url:
            fd = urlopen(self.image_url)
            f = io.BytesIO(fd.read())
            color_thief = ColorThief(f)

        if color_thief:
            color = color_thief.get_color(quality=1)
            self.color_hex = "#{:X}{:X}{:X}".format(color[0], color[1], color[2])
...
def resolve_node_type(node_type):
    if node_type in settings.ENTIFY_NODE_TYPES:
        node_model = import_string(settings.ENTIFY_NODE_TYPES[node_type])
        return node_model
...
class DjangoForm(Entity):
    ...
    def to_data(self, level, *args, **kwargs):
        obj = super(DjangoForm, self).to_data(level, *args, **kwargs)
        obj['type'] = 'form'
